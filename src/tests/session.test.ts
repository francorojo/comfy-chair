import { Session, SessionType, SessionSelection, SessionState } from "@app/session";
...
describe('test session case use', () => {
    ...
    test('Create a new session with more articles allowed', () => {
        ...
        expect( () => { session.addArticle(article)} ).toThrow(new Error("The number of items exceeds the maximum allowed"));
    });
    ...
    test('Create a new session, update state and try add new article', () => {
        ...
        expect( () => { session.addArticle(article)} ).toThrow(new Error("This session can not recive more articles"));
    });
